...
######################################################################
# Converts euler values to degrees
def euler_to_degrees(euler):
    x = ( (euler[0]) / (2 * math.pi) ) * 360
    y = ( (euler[1]) / (2 * math.pi) ) * 360
    z = ( (euler[2]) / (2 * math.pi) ) * 360
...
def convert(pose):
    ...
    degrees = euler_to_degrees(euler)
    return degrees
######################################################################
    ...
    for i, detection in enumerate(detections):
        ...
        if verbose==3:
            print(detection.tostring(collections.OrderedDict([('Pose',pose),
                                                          ...
                                                          ('FinalError', e1)]),
                                                          indent=2))
            print('x: {}, y: {}, z: {}'.format(degrees[0], degrees[1], degrees[2]))
            print()
